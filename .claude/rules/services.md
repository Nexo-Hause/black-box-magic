...
5. **Nunca ejecutar comandos que impriman URLs de remotes Git** (`git remote -v`, `git remote show`, `git config --get-regexp remote\..*\.url`). Si el remote fue clonado con PAT embebido (`https://x-access-token:ghp_...@github.com/...`), esos comandos filtran el token al transcript. Para probar conectividad: `git ls-remote origin HEAD` (éxito/fallo sin imprimir URL). Para ver qué remotes existen: `git remote` sin flags.
6. **No escribir scripts ad-hoc en `/c/tmp/` ni heredocs para leer secrets**. El hook `guard-secrets.sh` bloquea el patrón `readFileSync|open\(|File.read|Get-Content + access-token|.env|credentials` aunque esté dentro de un heredoc o envuelto en `sh -c`. Usar el capture assignment documentado en la sección Supabase (export VAR=$(node -e "...") como comando completo) o `op read`.
